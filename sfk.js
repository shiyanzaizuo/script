...
下载地址  https://raw.githubusercontent.com/age174/-/main/38600B49-DE8D-40CE-9BD2-09825DE8FB95.jpeg
...
1 * * * * https://raw.githubusercontent.com/age174/-/main/sfk.js, tag=刷福卡, img-url=https://raw.githubusercontent.com/age174/-/main/38600B49-DE8D-40CE-9BD2-09825DE8FB95.jpeg, enabled=true
...
*/
const $ = new Env('刷福卡');
let sfkurl = $.getdata('sfkurl')
let sfkhd = $.getdata('skfhd')
let sfkbody = $.getdata('sfkbody')
!(async () => {
...
function fkck() {
   if ($request.url.indexOf("openBall") > -1) {
    const sfkurl = $request.url
  if(sfkurl)     $.setdata(sfkurl,'sfkurl')
    $.log(sfkurl)
    const sfkhd = JSON.stringify($request.headers)
        if(sfkhd)    $.setdata(sfkhd,'sfkhd')
    $.log(sfkhd)
    
      const sfkbody = $request.body
  if(sfkbody)     $.setdata(sfkbody,'sfkbody')
    $.log(sfkbody)
   $.msg($.name,"","刷福卡获取成功！")
...
function sfk(timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      if (typeof $.getdata('sfkhd') === "undefined") {
        $.msg($.name,"",'请先获取刷福卡数据')
        $.done()
      }
      let url = {
        url : 'https://ddaydshuibaojk.tiantianzf.cn/blessCard/openBall',
        headers : JSON.parse($.getdata('sfkhd')),
        body :  sfkbody
      }
